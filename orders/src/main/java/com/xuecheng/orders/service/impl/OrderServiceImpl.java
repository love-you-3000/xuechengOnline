...
import com.xuecheng.orders.entity.XcOrders;
import com.xuecheng.orders.entity.XcOrdersGoods;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        if (order == null) XuechengException.cast("创建订单失败！");
        if (order.getStatus().equals("600002")) XuechengException.cast("订单已支付");
        XcPayRecord payRecord = createPayRecord(order);
        //生成二维码
        String qrCode = null;
        try {
    ...
    public XcOrders getOrderByBusinessId(String id) {
        LambdaQueryWrapper<XcOrders> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(XcOrders::getOutBusinessId, id);
